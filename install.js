...
if (process.platform !== 'darwin' && process.platform !== 'linux' && process.platform !== 'win32') {
	...
	console.error(`Prebuilt watchexec binaries are only provided for Linux, macOS and Windows, not for ${
		platformName()
	}.`);
	process.exit(1);
}
...
if (arch() === 'x86') {
	console.error('watchexec-bin doesn\'t support 32 bit architecture.');
	process.exit(1);
}
...
const {bin: {watchexec: binName}, name, version: pkgVersion} = require('./package.json');
...
(async () => {
	try {
		const [request, {Unpack}] = await Promise.all([
			...
			rejectUnsatisfiedNpmVersion('6.4.1')
		]);
